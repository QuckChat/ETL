...
- Traceback at `src/pipeline/ingestion.py` with `from __future__ import annotations`.
  - These usually indicate local file corruption/stale edits (or unresolved merges).
  - Run:

```powershell
python scripts/doctor.py --fix
# or manual restore:
# git restore --source=HEAD --worktree -- src/pipeline/*.py
# python scripts/doctor.py
```

If the issue persists after `--fix`, run `git pull --ff-only`, close your editor, and run `python scripts/doctor.py --fix` again.
